...
def test_dump_settings__success(tmp_path, monkeypatch):
    dummy_settings_path = tmp_path / "settings.json"
    monkeypatch.setattr("armasec_cli.config.settings_path", dummy_settings_path)
    dump_settings(
        init_settings(
            oidc_domain="test.domain",
            oidc_audience="https://test.domain/api",
...
def test_attach_settings__success(tmp_path, monkeypatch, mock_context):
    dummy_settings_path = tmp_path / "settings.json"
    monkeypatch.setattr("armasec_cli.config.settings_path", dummy_settings_path)
    dump_settings(
        init_settings(
...
def test_attach_settings__raises_Abort_if_settings_file_is_not_found(monkeypatch, mocker):
    monkeypatch.setattr("armasec_cli.config.settings_path", Path("fake-path"))
